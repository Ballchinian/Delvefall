...
- Every dealt card says where it stands rather than printing a raw score: "#3 most unique card in Magic" in the top ten, "more original than 85.2% of Magic cards" below that, counted against commander-legal cards unless illegal ones are switched on
...
The page ranks on the average of `cards.uniqueness` and `cards.concept_uniqueness`, the same measure taken over tags. A card with no tags has no concept score, so it ranks on rules text alone rather than scoring zero, the same rule a search follows for a tagless line. What gets printed is a standing rather than that average, whose top is 0.52: a rank in the top ten, otherwise the share of commander-legal cards a card beats, floored so 11th of 31,295 reads 99.9% and never 100%. It is counted per request (a 26ms scan) rather than stored, because two separate pipelines write the two scores. The scores are 32-bit floats, so a card whose every line and tag exists elsewhere lands a step either side of zero, and everything under 1e-6 counts as one tie: about 4,400 cards, printed with no number at all.
