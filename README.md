...
- Filters: color identity in three readings (at most these colors like deckbuilding, exactly these, or including these), price range, mana value range, card types (pick several, any of them matches, so an Artifact Creature answers to both), commanders only (legendary creatures, front face), hide game changers, and cards that aren't commander legal stay hidden unless you tick "include illegal"
- Results below 80% match (70 where the slider blends both axes) wait behind a "show weaker matches" button at the end of the list, so weak coincidences never crowd out real matches but nothing is unreachable. There is no knob for it: a control for "how similar" on a site about similarity was bloat, and the split's real job is keeping the price sorts meaningful
