...
- Tag picker: the concepts side of the slider shows the community tags it is searching, and clicking one switches it off, so a card that happens to be tagged for four different things can be searched for the one you care about. Switching a tag off takes the tags it implied with it, and the score is renormalized over what's left, so the percent keeps meaning what it always meant. Picking a line can narrow the tags to that one ability too, but that half is dark until its accuracy earns it: `ingest/attribute.py` infers which tags belong to which line and currently reads 88% precision, 82% recall, so a picked line still sets aside tags it shouldn't. It only runs where the `LINE_TAGS` environment variable is set
