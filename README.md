...
- Every dealt card says where it stands rather than printing a raw score: "#37 most unique card in Magic" in the top 100 the page lists, "more original than 85.2% of Magic cards" below that, counted against commander-legal cards unless illegal ones are switched on
...
The page ranks on the average of `cards.uniqueness` and `cards.concept_uniqueness`, the same measure taken over tags. A card with no tags has no concept score, so it ranks on rules text alone rather than scoring zero, the same rule a search follows for a tagless line. What gets printed is a standing rather than that average, whose top is 0.52: a rank in the top 100, the same numbers the list on the page carries, otherwise the share of commander-legal cards a card beats, floored so 101st of 31,295 reads 99.6% rather than rounding up to 99.7%. It is counted per request (a 26ms scan) rather than stored, because two separate pipelines write the two scores. The scores are 32-bit floats, so a card whose every line and tag exists elsewhere lands a step either side of zero, and everything under 1e-6 counts as one tie: about 4,400 cards, printed with no number at all.
...
The nearest neighbor scans walk an HNSW index, which turned 200-250ms of vector math per line into about 20ms. The build is deliberately denser than pgvector's defaults (m=64, ef_construction=400): common lines put hundreds of identical embeddings into the graph, and those copies link mostly to each other, so a search for a line next to one of them can get stuck inside the group. At m=32, 55 to 259 of the 35,987 distinct lines missed their best match depending on the build; at m=64 none did, checked line by line against the exact scan. Searches use pgvector's iterative scan in strict order, so a heavily filtered search keeps walking the graph until it has real answers instead of coming back short.
