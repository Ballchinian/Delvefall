#---- axis 2: conceptual similarity from the community tags ----
#a cosine over idf-weighted tag vectors, so rare tags count hard (sharing
#"wheel" means far more than sharing "removal"). the weights (tags.idf) and each
#card's vector length (card_tag_norms) are baked at ingest so queries stay cheap.
#
#the gate is written in DISPLAYED units on purpose: the map below is what changes
#when the scorer improves, the promise "80 means a good match" never does

#raw cosine -> displayed percent. the band is compressed (real matches ~0.5-0.7,
#related families ~0.2-0.35), and the anchors are fitted to the ROLLED UP vectors
#ingest/tags.py builds, where a pair shares mass through the whole tag tree
#rather than on exact tags only:
...
#  0.13 and below                shared-tag noise, the one anchor not measured
#eight judged pairs is a thin seed, so finetune/exam_concepts.py keeps the map
#honest as axis-2 reports accumulate
CALIBRATION = [(0.0, 0), (0.13, 35), (0.26, 55), (0.45, 70), (0.59, 82), (0.68, 90), (1.0, 100)]
MIN_CONCEPT = 80


def to_display(raw):
    #monotone, so orderings survive the translation
...
def from_display(pct):
    #the map walked backwards, so a displayed gate becomes a raw sql cutoff
...
def top_matches(conn, oracle_id, limit=20):
    #the join only ever touches the ANCHOR's tags, the norms folding the rest of
    #each candidate vector in, so this stays cheap over the whole pool
    return conn.execute("""
        WITH anchor AS (
